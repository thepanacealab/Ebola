...
Sentimentscores_who <- read.csv(text="day,month_year,sentiment,score")
...
for (row in 1:nrow(news)) {
  ...
  sentiment_who<-get_nrc_sentiment((text))
  #calculationg total score for each sentiment
  Sentimentscores<-data.frame(colSums(sentiment_who[,]))
  
  names(Sentimentscores)<-"Score"
  Sentimentscores<-cbind("day"=rep(current_date[1],10),"month_year"=rep(month_year,10),"sentiment"=rownames(Sentimentscores),Sentimentscores)
  rownames(Sentimentscores)<-NULL
  
  Sentimentscores_who <- rbind(Sentimentscores_who, Sentimentscores)
}

head(Sentimentscores_who)
names(Sentimentscores_who)

agg_sentiment <- aggregate(Sentimentscores_who$Score, by=list(date=Sentimentscores_who$month_year, sentiment=Sentimentscores_who$sentiment), FUN=sum)
...
p <- ggplot(data=new,aes(x=date,y=Score, group=sentiment))+ geom_line(aes(color=sentiment))+
  ...
  xlab("Days")+ylab("scores")+ggtitle("Sentiments nrc - source WHO")
...
q <- ggplot(data=new2,aes(x=date,y=Score, group=sentiment))+ geom_line(aes(color=sentiment))+
  geom_point(aes(color=sentiment))+
  theme(legend.position="right")+
  xlab("Day")+ylab("scores")+ggtitle("Anticipation/Fear sentiments plot - WHO")
